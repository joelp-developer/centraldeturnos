...
export class FirebaseService {
  constructor(private router: Router, private baseSQLService: BaseSQLService) {}
  ...
  async login(email: string, password: string): Promise<boolean> {
    ...
    try {
      ...
      const data: any = await this.baseSQLService
        .getbyUsuario(email)
        .toPromise();
      console.log(data);
      if (data && data.IdTipoUsuario) {
      ...
      updatePassword(this.auth.currentUser, newPassword)
        .then((data) => {
          ...
          if (this.auth.currentUser?.email) {
            this.baseSQLService.putUsuariopass(
